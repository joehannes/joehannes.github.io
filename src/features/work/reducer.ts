...
const addOrUpdate = (state: TWorkState, action: TWorkAction, selector: keyof TWorkState): TWorkState => {
  if ((state[selector] as Array<any>).filter(selected => selected.id === action.payload.id).length === 0) {
    (state[selector] as Array<any>).push(action.payload);
  } else {
      const i = (state[selector] as Array<any>).findIndex(selected => selected.id === action.payload.id);

      (state[selector] as Array<any>).splice(i, 1, { ...action.payload });
  }
...
const reducer = createReducer<TWorkState, TWorkAction>(initialState, {
  [UPDATE_WORK_EXPERIENCE]: (state, action) => addOrUpdate(state, action, "workExperience"),
  ...
  [UPDATE_TECHNOLOGY]: (state, action) => addOrUpdate(state, action, "technology"),
  [UPDATE_CONTRACT_TYPE]: (state, action) => addOrUpdate(state, action, "contractType"),
