...
const WorkExperience: FC = () => {
  const selectWorkExperience = (state: RootState) => {
    const { workExperience, company, contractType, location, position, project, technology } = state.work;
    ...
    const _workExperience = workExperience.map(w => ({
      ...w,
      technology: w.technology.map(t => technology.find(_t => t.includes(_t.id))),
    ...
    <Timeline>
      {jsonData.map((job: any, i: number) => (
        <TimelineElement
          ...
          contentArrowStyle={{ borderRight: `7px solid  ${job.company.icon.color}` }}
          date={describeDateRange(job.date)}
          iconStyle={{...job.style}}
          icon={<Icon color={job.company.icon.color as any} name={job.company.icon.id as any} size="big" className={styles.icon}/>}
        >
          <Label
            as='a'
          ...
          <br/><br/>
          <h3 className="vertical-timeline-element-title">{job.position.title} @{job.company.title}</h3>
          <h4 className="vertical-timeline-element-subtitle">
            ...
            <br/>
            #{job.description.function}</h4>
          <Segment.Group>
            <Segment basic className={styles.label}>
              <Label color={job.company.icon.color as any} horizontal>
                company size
              </Label>
              <Rating icon='star' defaultRating={job.company.size} maxRating={7} disabled />
            </Segment>
            <Segment basic className={styles.label}>
              <Label color={job.company.icon.color as any} horizontal>
                method
            ...
            {job.company.url&& (<Segment basic>
              <Button icon="world" href={job.company.url} as="a" target="_blank" rel="noreferrer"/>
            </Segment>)}
          </Segment.Group>
        </TimelineElement>
      ))}
